...
Import this package with a npm alias because postman wants the package name as `newman-reporter-csv` so do this

```
npm install newman-reporter-csv@npm:@cardiosmircem/newman-reporter-csv@latest
```

**if you do not import it with the newman-reporter-csv alias it will not work**
...
| executed        | test names that passed _(comma separated)_                     | Status was 200, User was created  |
| failed          | test names that failed _(comma separated)_                     | User has view permissions         |
| skipped         | test names that were skipped _(comma separated)_               | User had first name Joe           |
| body            | the response body _(optional column. see [Options](#options))_ | { foo: "bar" }                    |
